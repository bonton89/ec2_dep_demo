...
if not pdf_text:
    ...
    if st.button("Generate Response", key="generate_btn"):
        ...
        with st.spinner("Generating Model output please wait..."):
            ...
            #st.markdown(resp)
            st.dataframe(df[["Sl No","Prompt Query","System Data","LLM Response","Source"]],hide_index=True)
...
st.markdown('<div class="sub-header">Prompt Template </div>', unsafe_allow_html=True)

# Showing prompts and instructions
# Inject CSS for blue button
st.markdown("""
    <style>
    div.stButton > button {
...
if st.button("Show/Hide Prompts Used for Plan Audit", key="toggle_button"):
    st.session_state.show = not st.session_state.show

if st.session_state.show:
    st.markdown(prompt_template, unsafe_allow_html=True)
